...
import {
  useProcesses,
} from "@/app/api/hooks/processes/useProcesses";
...
import { Activity, ActivityType } from "@/app/api/hooks/process/useCreateActivity";
...
export default function KanbanDashboard() {
  ...
  const apiFilters = useMemo(() => {
    ...
    if (filters.hasNewMovementsNow !== undefined && filters.hasNewMovementsNow) {
  ...
  const handleExportWithColumns = async (selectedColumns: string[], exportAll: boolean) => {
    try {
      ...
      if (exportAll) {
        ...
        const maxPages = Math.ceil(totalProcesses / SAFE_LIMIT);
          ...
          for (let page = 1; page <= maxPages; page++) {
            ...
            while (retries <= MAX_RETRIES && !pageSuccess) {
              ...
              const errorMessage = pageError?.response?.data?.message || pageError?.message || 'Erro desconhecido';
              const statusCode = pageError?.response?.status || 'desconhecido';
              
              console.error(`❌ Erro ${statusCode} na página ${page} (tentativa ${retries}/${MAX_RETRIES + 1}):`, errorMessage);
              
              // Se esgotou todas as tentativas, PARA COMPLETAMENTE
              if (retries > MAX_RETRIES) {
                console.error(`💥 PARANDO EXPORTAÇÃO - Todas as tentativas falharam na página ${page}`);
                
                // Check for MongoDB memory error
                const isMemoryError = errorMessage.includes('memory limit') || errorMessage.includes('allowDiskUse');
          ...
          if (!pageSuccess) {
            console.error(`💥 ERRO CRÍTICO: Página ${page} não foi carregada após todas as tentativas`);
            throw new Error(
        ...
        if (processesToExport.length === 0) {
          throw new Error("Nenhum processo foi carregado. Verifique a conexão com o servidor.");
        }
      ...
      await exportToExcel(processesToExport, "processos-lista", selectedColumns);
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Não foi possível exportar os processos. Tente novamente.",
        variant: "destructive",
      });
    }
  };
  ...
  const getFilteredActivities = (process: Process): Activity[] => {
    ...
    if (user?.role === UserRolesEnum.ADMIN) {
      ...
      return activities.filter(activity => {
        ...
        const assignedToId = typeof activity.assignedTo === 'string' 
          ? activity.assignedTo 
          : activity.assignedTo?._id;
        
        // Verificar se o usuário completou a atividade
        const completedById = typeof activity.completedBy === 'string' 
          ? activity.completedBy 
          : activity.completedBy?._id;
        
        // Verificar se o usuário atribuiu a atividade
        const assignedById = typeof activity.assignedBy === 'string' 
          ? activity.assignedBy 
  ...
  const someVisibleSelected = useMemo(() => {
    if (selectedProcessIds.size === 0 || filteredProcesses.length === 0) return false;
    return filteredProcesses.some(p => selectedProcessIds.has(p._id)) && !allVisibleSelected;
  }, [filteredProcesses, selectedProcessIds, allVisibleSelected]);

  // Auto-detect when all visible are selected and update mode
  useEffect(() => {
    if (allVisibleSelected && filteredProcesses.length > 0 && selectedProcessIds.size === filteredProcesses.length && selectAllMode !== 'all') {
      setSelectAllMode('page');
    } else if (selectedProcessIds.size === 0) {
      setSelectAllMode(null);
    }
  }, [allVisibleSelected, filteredProcesses.length, selectedProcessIds.size, selectAllMode]);
  ...
  const normalizedFilters = useMemo(() => ({
    search: typeof filters.search === "string" ? filters.search : String(filters.search || ""),
    status: typeof filters.status === "string" ? filters.status : String(filters.status ?? "all"),
    type: typeof filters.type === "string" ? filters.type : String(filters.type ?? "all"),
    startDate: typeof filters.startDate === "string" ? new Date(filters.startDate) :
      typeof filters.startDate === "object" && filters.startDate !== null && !Array.isArray(filters.startDate) ?
        (filters.startDate as Date) : undefined,
    endDate: typeof filters.endDate === "string" ? new Date(filters.endDate) :
      typeof filters.endDate === "object" && filters.endDate !== null && !Array.isArray(filters.endDate) ?
        (filters.endDate as Date) : undefined,
    lossReason: typeof filters.lossReason === "string" ? filters.lossReason : String(filters.lossReason ?? "all"),
    contentFilter: typeof filters.contentFilter === "string" ? filters.contentFilter : String(filters.contentFilter ?? "all"),
    ...
    >
      <main className="w-full px-3 sm:px-4 lg:px-6 xl:px-8 py-8">
        <>
            <div className="mb-8">
              <FiltersBar
                filters={{
                  ...
                  hasNewMovementsNow: Boolean(normalizedFilters.hasNewMovementsNow),
                }}
                onFiltersChange={(newFilters) => {
                  Object.entries(newFilters).forEach(([key, value]) =>
                    setFilter(key, value as string | number | boolean | null | undefined)
                  );
                }}
                onApplyFilters={() => {
            ...
            {isLoading && (filters.search || filters.status !== "all" || (filters.type && filters.type !== "all") || 
             (filters.lossReason && filters.lossReason !== "all") || filters.startDate || filters.endDate || 
             filters.emptyDocuments || filters.emptyInstances || filters.hasNewMovementsNow) && (
              <div className="space-y-8">
                ...
                <div className="backdrop-blur-sm rounded-2xl border shadow-lg overflow-hidden bg-white/80 dark:bg-gray-800/80 border-gray-200 dark:border-gray-700">
                  ...
                  <div className="p-6 space-y-3">
                    {[...Array(8)].map((_, i) => (
                      <div key={i} className="h-16 rounded-lg animate-pulse bg-gray-100 dark:bg-gray-700/50"></div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex justify-center py-8">
                  <div className="rounded-2xl shadow-lg p-6 border bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium text-gray-700 dark:text-gray-300">Aplicando filtros...</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {isLoading && page === 1 && !filters.search && filters.status === "all" && 
             (!filters.type || filters.type === "all") && 
             (!filters.lossReason || filters.lossReason === "all") && 
             !filters.startDate && !filters.endDate && 
             !filters.emptyDocuments && !filters.emptyInstances && !filters.hasNewMovementsNow ? (
              <div 
                className="backdrop-blur-sm rounded-2xl border shadow-lg overflow-hidden bg-white/80 dark:bg-gray-800/80 border-gray-200 dark:border-gray-700"
              >
              ...
              /* List/Table View */
                ...
                >
                  ...
                  {selectedCount > 0 && (
                    <div className="px-6 py-3 border-b flex items-center gap-3 bg-blue-50 dark:bg-blue-900/20 border-gray-200 dark:border-gray-700">
                      ...
                      {/* <Button
                        ...
                        onClick={() => {
                          ...
                          toast({
                            ...
                            description: "Envio de email em grupo será implementado em breve.",
                          });
                        }}
                      >
                      ...
                      <Button
                        ...
                        onClick={() => {
                          // TODO: Implement schedule activity
                          toast({
                            title: "Funcionalidade em desenvolvimento",
                            description: "Agendamento de atividade será implementado em breve.",
                          });
                        }}
                      >
                  ...
                  {selectAllMode === 'page' && allVisibleSelected && totalProcessesInDB > filteredProcesses.length && (
                    <div className="px-6 py-3 border-b flex items-center gap-3 bg-blue-50 dark:bg-blue-900/30 border-gray-200 dark:border-gray-700">
                      <span className="text-sm text-gray-700 dark:text-gray-300">
                        {filteredProcesses.length} processo{filteredProcesses.length !== 1 ? 's' : ''} selecionado{filteredProcesses.length !== 1 ? 's' : ''} nesta página.
                      </span>
                      <button
                  ...
                  {selectAllMode === 'all' && (
                    <div className="px-6 py-3 border-b flex items-center gap-3 bg-blue-50 dark:bg-blue-900/30 border-gray-200 dark:border-gray-700">
                      ...
                      <button
                        onClick={() => {
                          setSelectAllMode('page');
                          setSelectedProcessIds(new Set(filteredProcesses.map(p => p._id)));
                        }}
                        className="text-sm font-semibold underline text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300"
                      >
                        Limpar seleção
                      ...
                      <TableBody>
                        {filteredProcesses.length === 0 ? (
                          ...
                          filteredProcesses.map((process) => {
                            const isSelected = selectedProcessIds.has(process._id);
                            return (
                              <TableRow
                                key={process._id}
                              ...
                              >
                                <div className="flex flex-col gap-1">
                                  <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                                    {capitalizeWords(getProcessTitle(
                              ...
                              <TableCell onClick={() => window.location.href = `/processes/${process.number}`}>
                  ...
                  {(() => {
                    ...
                    const totalPages = data?.totalPages || paginationInfo.totalPages || 0;
      ...
      {showScrollTopButton && (
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24" className="group-hover:scale-110 transition-transform">
            <path stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M5 15l7-7 7 7" />
          </svg>
        </button>
      )}

...
import { getStageLabel, getEsteiraLabel } from "@/app/utils/processUtils";
...
import { getClaimant, getClaimantAttorney, getProcessTitle, getDefendant } from "@/app/utils/processPartsUtils";
...
export default function ProcessDetailsEditPage() {
  ...
  const getNewMovements = () => {
    ...
    const firstDegreeMovs = process.moviments.filter(m => m.instancia === InstanceEnum.FIRST_INSTANCE);
    const secondDegreeMovs = process.moviments.filter(m => m.instancia === InstanceEnum.SECOND_INSTANCE);
    ...
    const countNewFirst = oldFirst !== null ? Math.max(0, firstDegreeMovs.length - oldFirst) : 0;
    const countNewSecond = oldSecond !== null ? Math.max(0, secondDegreeMovs.length - oldSecond) : 0;
    const countNewTst = oldTst !== null ? Math.max(0, tstMovs.length - oldTst) : 0;
  ...
  const hasSecondDegreeMovements = process?.moviments?.some(m => m.instancia === InstanceEnum.SECOND_INSTANCE) || false;
  ...
  const isProcessError = !!error || (!isLoading && !isProcessLoaded) || (process && !hasRequiredData);
  ...
  const [activeRightTab, setActiveRightTab] = useState<"documents" | "activities">(initialRightTab);
  const [activeInstance, setActiveInstance] = useState<"1grau" | "2grau" | "tst">("1grau");
  ...
  const [showRemoveProvisionalLinkConfirm, setShowRemoveProvisionalLinkConfirm] = useState(false);
  const [showLinkProvisionalExecutionModal, setShowLinkProvisionalExecutionModal] = useState(false);
  ...
  const { insertExecution, isLoading: isInsertExecutionLoading, success: insertExecutionSuccess } = useInsertExecution();
  ...
  const handleDocumentClick = (documento: DocumentExtract) => {
    ...
    router.push(`/processes/${process.number}?tab=documents`, { scroll: false });
  };
  ...
  const handleMovementClick = (mov: any) => {
    ...
    const matchingDocs = process.documents.filter(doc => {
      ...
      return normalizedMovDate && normalizedDocDate && normalizedMovDate === normalizedDocDate;
    });
  ...
  const handleConfirmLinkProvisionalExecution = async () => {
    ...
    try {
      await insertExecution(process._id, executionNumberInput.trim(), executionNumberInput.trim());
  ...
  const handleStartEditTitle = useCallback(() => {
    ...
    if (currentTitle && currentTitle.trim() && currentTitle !== process?.number) {
  ...
  }, [process?.title, process?.formPipedrive?.title, process?.processParts, process?.documents, process?.number]);
  ...
  const handleClaimantChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    requestAnimationFrame(() => {
      if (claimantInputRef.current && cursorPosition !== null) {
        claimantInputRef.current.setSelectionRange(cursorPosition, cursorPosition);
      }
    });
  }, []);

  const handleDefendantChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const cursorPosition = e.target.selectionStart;
    ...
    requestAnimationFrame(() => {
      if (defendantInputRef.current && cursorPosition !== null) {
        defendantInputRef.current.setSelectionRange(cursorPosition, cursorPosition);
      }
    });
  }, []);
  ...
  }, [process?.number, editedClaimant, editedDefendant, updateProcessFormMutation, refetchProcess]);

  const handleSyncConfirm = async (options: { movements: boolean; documents: boolean }) => {
    try {
      if (!process?.number) {
        toast.error("Número do processo não encontrado.");
        return;
  ...
  const handleRejectUpdate = () => {
    ...
    toast.warning("Atualização cancelada. Seus dados não salvos foram preservados.", {
      position: "top-right",
      autoClose: 4000,
    });
  };
  ...
  useEffect(() => {
    ...
    if (isSyncing && !isCurrentlySyncing) {
      ...
      if (hasErrorStatus) {
        ...
        toast.success(`${getSyncStatusDescription(process.processStatus)} com sucesso!`, {
          position: "top-right",
          autoClose: 3000,
  ...
  useEffect(() => {
    if (process && lastProcessData && !isEditing) {
      ...
      const hasFormChanges =
        ...
        formState.processNumber !== (process?.formPipedrive?.processNumber || process?.number || "") ||
        formState.executionNumber !== (process?.formPipedrive?.executionNumber || process?.calledByProvisionalLawsuitNumber || "") ||
        formState.observacao !== (process?.observation?.description || process?.formPipedrive?.observacao || "");
  ...
  useEffect(() => {
    ...
    if (process?.formPipedrive) {
        ...
        defendants:
          process?.companies
            ?.map((c) => {
              ...
              if (typeof c.score === "number") solvencyParts.push(`Score: ${c.score}`);
              if (c.porte) solvencyParts.push(`Porte: ${c.porte}`);
              if (c.registrationStatus) solvencyParts.push(`Registro: ${c.registrationStatus}`);
              if (c.specialRule) solvencyParts.push(`Solvência: ${c.specialRule}`);
              const solvency = solvencyParts.length ? ` - ${solvencyParts.join(" | ")}` : "";
              return `${c.name} (${mascararCNPJ(c.cnpj)})${solvency}`;
            })
            .join(", ") || process?.formPipedrive?.defendants || "",
      ...
      const defendantsFromParts = Array.from(defendantNames).map(name => {
        const company = process?.companies?.find(c => 
          c.name.toLowerCase() === name.toLowerCase()
        );
        
        if (company) {
          const solvencyParts: string[] = [];
          if (typeof company.score === "number") solvencyParts.push(`Score: ${company.score}`);
          if (company.porte) solvencyParts.push(`Porte: ${company.porte}`);
          if (company.registrationStatus) solvencyParts.push(`Registro: ${company.registrationStatus}`);
          if (company.specialRule) solvencyParts.push(`Solvência: ${company.specialRule}`);
          const solvency = solvencyParts.length ? ` - ${solvencyParts.join(" | ")}` : "";
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("calc-note", handleCustomEvent as EventListener);
  ...
  return (
    <MainShell>
        ...
        onSync={async () => {
          ...
          if (!canSync(process?.processStatus, process?.synchronizedAt)) {
            if (process?.synchronizedAt) {
              ...
              toast.error("Só é permitido sincronizar caso synchronizedAt já tenha passado 30 minutos.");
            }
            return;
          }
            ...
            <div className="h-[500px] sm:h-[600px] lg:h-[calc(100vh-255px)]">
              {activeInstance === "1grau" && (
                <TimelineCard
                  ...
                  processNumber={process?.number}
                  newMovements={newMovements.PRIMEIRO_GRAU.map(mov => ({ ...mov, instancia: InstanceEnum.FIRST_INSTANCE }))}
              ...
              {activeInstance === "2grau" && (
                <TimelineCard
                  ...
                  processNumber={process?.number}
                  newMovements={newMovements.SEGUNDO_GRAU.map(mov => ({ ...mov, instancia: InstanceEnum.SECOND_INSTANCE }))}
                  onMovementClick={handleMovementClick}
                  documents={process?.documents || []}
                  onDocumentClick={handleDocumentClick}
          ...
          <div className="lg:col-span-4 h-[500px] sm:h-[600px] lg:h-[calc(100vh-200px)] order-2">
            <div className="h-full flex flex-col bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
              ...
              <div className="flex-1 min-h-0 overflow-hidden">
                {activeRightTab === "documents" ? (
            <DocumentsCard
              documents={linkedDocuments.length > 0 ? linkedDocuments : process?.documents || []}
      ...
      <Dialog open={showUpdateConfirmation} onOpenChange={setShowUpdateConfirmation}>
        <DialogContent>
          ...
          <div className="py-4 text-base">
            <p className="mb-3">
              Novos dados do processo foram detectados e estão disponíveis para atualização.
            </p>
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3">
              <p className="text-sm text-amber-700 dark:text-amber-300">
                <strong>Atenção:</strong> Você possui alterações não salvas no formulário.
                Se aceitar a atualização, suas alterações serão perdidas.
              </p>
            </div>
          </div>
      ...
      <Dialog open={showSyncCompleteDialog} onOpenChange={setShowSyncCompleteDialog}>
            ...
            {hasError(process?.processStatus) ? (
              <>
                ...
                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3 space-y-2">
                  ...
                  <div className="text-sm text-red-700 dark:text-red-300">
                    <strong>Status:</strong> {process?.processStatus?.name || ProcessStatusEnum.ERROR}
                  </div>
                  ...
                  {process?.processStatus?.updatedAt && (
                    <div className="text-sm text-red-700 dark:text-red-300">
                      <strong>Data:</strong> {new Date(process.processStatus.updatedAt).toLocaleString('pt-BR')}
                    </div>
                  )}
                </div>
              </>
              ...
              <>
                ...
                <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3 space-y-2">
                  ...
                  <div className="text-sm text-green-700 dark:text-green-300">
                    <strong>Status:</strong> {process?.processStatus?.name || ProcessStatusEnum.PROCESSED}
                  </div>
                  <div className="text-sm text-green-700 dark:text-green-300">
                  ...
                  {process?.synchronizedAt && (
                    <div className="text-sm text-green-700 dark:text-green-300">
                      <strong>Sincronizado em:</strong> {new Date(process.synchronizedAt).toLocaleString('pt-BR', {
                        day: '2-digit',
                        month: '2-digit',
                        year: 'numeric',
                        hour: '2-digit',
                ...
                <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                  <p className="text-sm text-blue-700 dark:text-blue-300 font-medium">
                    {process?.processStatus?.name === ProcessStatusEnum.EXTRACTION_FINISHED
      ...
      <Dialog open={showProcessInfoModal} onOpenChange={setShowProcessInfoModal}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {process?.class && (
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="p-2.5 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                      <span className={`text-xs font-medium ${process.class === "MAIN" ? 'text-blue-600 dark:text-blue-400' : 'text-amber-600 dark:text-amber-400'
                        }`}>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowProcessInfoModal(false)}>
              Fechar
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={showRemoveProvisionalLinkConfirm} onOpenChange={setShowRemoveProvisionalLinkConfirm}>
        <DialogContent className="max-w-md">
          ...
          <div className="py-4">
            <p className="text-base text-gray-700 dark:text-gray-300 mb-4">
              Tem certeza que deseja remover o vínculo com a execução provisória?
            </p>
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3">
              <p className="text-sm text-amber-700 dark:text-amber-300">
                <strong>Atenção:</strong> Esta ação irá desvincular o processo da seguinte execução provisória:
              </p>
      ...
      <Dialog open={showLinkProvisionalExecutionModal} onOpenChange={setShowLinkProvisionalExecutionModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
          ...
          <div className="py-4 space-y-4">
            <p className="text-base text-gray-700 dark:text-gray-300">
              Insira o número da execução provisória que deseja vincular a este processo.
            </p>
            <div className="space-y-2">
              <Label htmlFor="execution-number" className="text-sm font-semibold">
            ...
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
              <p className="text-sm text-yellow-700 dark:text-yellow-300">
                <strong>Atenção:</strong> Certifique-se de que o número da execução provisória está correto antes de vincular.
              </p>
            </div>
          </div>
          <DialogFooter className="flex gap-2 justify-end">
            ...
            <Button
              ...
              disabled={isInsertExecutionLoading || !executionNumberInput.trim()}
